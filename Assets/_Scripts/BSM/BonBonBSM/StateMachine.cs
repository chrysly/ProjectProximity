...
//Repurposed for Dive
//Stolen by Bonbon
//Defiled by Jasmine for game studio ;__;
    ...
    where I : StateInput
{
    ...
    private Dictionary<Type, S> _stateMap = new Dictionary<Type, S>();  // not sure what Type is yet

    private IEnumerator _transitionAction = null;   // i assume between state transitions do X
    private bool _locked = false;   // unsure

    public event Action StateTransiton; // (State, PrevState)   // unsure

    #region Unity Messages

    protected virtual void Awake() => CurrInput = (I) Activator.CreateInstance(typeof(I));  // unsure
